...
def main():
    groups = [
        ("ficheiros exemplo", run_example_files),
        ("construcao da tabela", run_table_cases),
        ("erros sintaticos/lexicos", run_invalid_syntax_cases),
        ("validacoes semanticas validas", run_valid_semantic_cases),
        ("erros semanticos", run_invalid_semantic_cases),
        ("varios erros semanticos", run_multi_error_case),
        ("linhas em erros semanticos", run_line_error_cases),
    ]
    ...
    total = (
        len(EXAMPLE_FILES)
        + len(TABLE_CASES)
        + len(INVALID_SYNTAX_CASES)
        + len(VALID_SEMANTIC_CASES)
        + len(INVALID_SEMANTIC_CASES)
        + 1
        + len(LINE_ERROR_CASES)
    )
    print(f"\nTodos os testes passaram ({total} casos).")
